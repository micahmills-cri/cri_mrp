...
```sql
- id                      UUID PRIMARY KEY
- order_number            TEXT UNIQUE NOT NULL    -- e.g., "WO-2026-0123"
...
*If status is PENDING_REVIEW or RELEASED (not started):*
...
*If status is IN_PROGRESS, ON_HOLD, COMPLETED:*
...
**Tab 4: Notes & Attachments** *(Collapsible)*
...
```json
{
  ...
  "details": {
    "existingOrderNumber": "WO-2026-0100"
  }
}
```
...
- Product SKU (text input or dropdown of common SKUs)
- Build Spec (JSON editor or form fields)
...
9. Confirmation dialog: "Release WO-2026-0123 to production floor?"
10. On confirm:
...
2. **View Queue:**
   - Available Work section shows 3 work orders (FIFO):
     - WO-2026-0120 (PRIORITY: NORMAL)
     - WO-2026-0118 (PRIORITY: HIGH) ← should work this first
     - WO-2026-0119 (PRIORITY: NORMAL)
   - Upcoming Work section shows 2 work orders (start date: tomorrow)

3. **Select Work:**
   - Operator taps "Iniciar Trabajo" (Start Work) on WO-2026-0118
...
11. **Next Station Sees Work:**
    ...
    - WO-2026-0118 now appears in their Available Work queue
...
1. Operator is clocked into WO-2026-0125 (Active Time: 00:45:00)
2. Realizes epoxy needs to cure for 2 hours before next step
3. Taps "PAUSAR"
4. Returns to My Queue
5. Selects different work order: WO-2026-0127
6. Taps "Iniciar Trabajo"
7. Attempts to clock on
8. **Error:** "Ya estás registrado en WO-2026-0125. Por favor, cierra sesión antes de comenzar un nuevo trabajo."
   - (Translation: "You are already clocked into WO-2026-0125. Please clock off before starting new work.")
9. Operator returns to WO-2026-0125
10. Taps "COMPLETAR PASO" (since waiting, marks this stage done)
11. Step completed, time entry ends
12. Returns to queue
13. Now can start WO-2026-0127 successfully

**Alternate Approach (if pause doesn't complete step):**
- System allows "switch" if current job is paused
- But recommends completing paused job first to avoid confusion
...
1. Supervisor in Active Work section sees WO-2026-0130 in progress
2. Receives call: "Problem with hull alignment on 0130"
...
2. Supervisor opens WO-2026-0130 from Active Work (still shows held orders)
...
2. Search returns work order: WO-2024-0789 (completed 2.5 years ago)
3. Click "View" → Work Order Detail Modal (read-only)
4. Review Build Spec tab: LX22-SPORT-2024, customer ref, features
...
8. **Inventory Integration** - Track parts consumption per step, trigger reorders
9. **Multi-Facility Support** - Expand to multiple factory locations
10. **Mobile App** - Native iOS/Android apps for operators (better than web on tablets)
11. **Offline Mode** - Operators can clock on/off without internet, sync later
12. **Predictive Analytics** - ML model to predict completion dates based on historical data
13. **Video Work Instructions** - Embedded video player with timestamped steps
14. **Voice Input** - Operators add notes via voice-to-text (hands-free)
15. **Automated Backups to External Storage** - Daily automated sync to customer's own backup location
