  ...
  try {
    ...
    const statusCounts = {
      ...
      COMPLETED: filteredWorkOrders.filter(
        (wo) => wo.status === 'COMPLETED' && wo.woStageLogs.some((log) => log.createdAt >= today)
      ).length,
      ...
      for (let i = 0; i < stageLogs.length - 1; i++) {
        ...
        if (
          ...
          const timeInMinutes = Math.floor(
            (endLog.createdAt.getTime() - startLog.createdAt.getTime()) / (1000 * 60)
          )
    ...
    if (workCenterFilter) {
      finalWipData = wipData.filter((wo) => {
        ...
        return (
          ...
          wo.currentStage.workCenter.name.toLowerCase().includes(workCenterFilter.toLowerCase())
        )
      })
    }
