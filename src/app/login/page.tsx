...
export default function LoginPage() {
  ...
  const handleLoginRequest = async ({ email, password }: { email: string; password: string }) => {
    setError('')
    ...
    <div className="min-h-screen flex items-center justify-center bg-background px-4">
      <div className="bg-white p-8 rounded-lg shadow-sm border border-slate-200 w-full max-w-md">
        <div className="text-center mb-8">
          <h1 className="text-2xl font-semibold text-slate-900 mb-2">Boat Factory Operations</h1>
          <p className="text-sm text-slate-600">Sign in to access your manufacturing dashboard</p>
        </div>
        ...
        {devMode ? (
          <div className="space-y-6">
            ...
            <div className="p-4 bg-slate-50 rounded-md border border-slate-200">
              <h3 className="text-sm font-semibold text-slate-700 mb-3">Demo Accounts</h3>
              <div className="space-y-2">
                {testAccounts.map((account) => (
                  <Button
                    ...
                    disabled={!!quickLoginRole && quickLoginRole !== account.role}
                    onClick={() =>
                      handleQuickLogin({
                        email: account.email,
